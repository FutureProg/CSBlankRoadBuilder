...
{
	public override string TabName { get; } = "Custom Markings";
	...
	private static string GetDescription(GenericMarkingType key)
	{
		switch (key)
		{
			case GenericMarkingType.End:
	...
	private void AddLineOption(GenericMarkingType key, SavedLineOption value)
	{
		...
		UILabels.AddLabel(_panel, Margin + topLabel.width + Margin, yPos - topLabel.height - 6, GetDescription(key)).textColor = new Color32(200, 200, 200, 255);
		...
		static Dictionary<int, string> getEnumValues() => Enum.GetValues(typeof(MarkingLineType)).Cast<MarkingLineType>().ToDictionary(x => (int)x, x => x.ToString().FormatWords());
